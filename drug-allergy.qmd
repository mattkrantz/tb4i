...
data <- data %>% group_by(protocol) %>% arrange(protocol, time) %>% mutate(cumulative_time = cumsum(time))

ggplot(data, aes(x = cumulative_time, y = amount_per_min, color = protocol, fill=protocol)) + geom_area(alpha=0.5,position="identity") + geom_line(aes(color=protocol),size=1) + scale_fill_manual(values = c("gray50", "gray80")) + scale_color_manual(values = c("gray50", "gray80")) + labs(title = "Cumulative Dose Over Time", x = "Time (minutes)", y = "Rate (mg/min)") + theme_minimal() + theme(legend.title = element_blank()) + theme_classic()
