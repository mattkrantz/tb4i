---
title: "On Being a Physician-Scientist"
execute:
  echo: false
  message: false
  warning: false
...
library(dplyr)
library(here)
dat = read_csv(here("data", "nih-k-awards-activity-total.csv"))

# Convert the percentage values from character to numeric
dat$success_rate <- as.numeric(sub("%", "", dat$success_rate))
...
viewof selected_nih_institute = Inputs.select(["ACTIVITY TOTAL"].concat(nih_institutes), {label: "NIH Institute"})
...
plt2 = Plot.plot({
  color: {legend: true, label: "Activity Code"},
  marks: [
    Plot.barY(filtered, {x: "fiscal_year", y: "n_applications", fill: "activity_code", tip: true})
    ],
